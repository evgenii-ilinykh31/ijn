...
function moveTouchStartListener(note, id, startListener) {
    ...
    //подготовка под извлечение заметки
    ...
    //извлечение заметки
    let currentPlug = false;
    let previousPlug = extractNoteFromFlowJobAndCreateInsertGetPlug(note, notesCurrentArray, noteWidth, noteHeight, noteMarginTop, noteMarginBottom, topCoordinate);
    let passedNote = false;
    let timer = false;
    let moveListener = function (event) {
        //убираем то, что не должно сработать пока курсор не остановился
        if (timer) {
            clearTimeout(timer);
        }

        //записываем passedNote, если она есть
        let currentNote = getCurrentNote(touchStartX, currentY, note.id);
        passedNote = currentNote ? currentNote : passedNote;

        //прибавляем количество px к текущему положению
        currentY = isMobile() ? event.changedTouches[0].clientY : event.clientY;
        note.style.top = (topCoordinate - (touchStartY - currentY)) + 'px';

        //отправляем в заморозку то, что не должно выполняться без остановки мышки
        timer = setTimeout(() => {
            currentPlug = getCurrentPlug(currentY, noteHeight, noteMarginTop, flowJob, cssPlug);

            //проверяем наличие plug в межцентровой area и его соответствие previousPlug(переменной previousPlug изначально присваивается ссылка на plugOnTouch)
            //если да - return(мы еще не вышли за пределы области, где создана заметка)
            if (currentPlug && currentPlug === previousPlug) {
                return;
            }

            //проверяем наличие plug в межцентровой area
            //если да - ставим ему высоту 100% от заметки
            //если нет - создаем/вставляем
            if (currentPlug) {
                currentPlug.style.height = noteHeight + 'px';
            } else if (passedNote) {
                currentPlug = createNotePlug(noteWidth, 0, noteMarginTop, noteMarginBottom);   //0 потому, что мы вставляем невидимую заглушку, потом ее увеличиваем;
                insertPlugToFlow(flowJob, passedNote, currentY, notesCurrentArray, currentPlug, noteHeight + noteMarginTop);
            }

            //previousPlug ставим высоту 0 и setTimeout на удаление(который удаляет только в том случае, если высота previousPLug = 0)
            previousPlug.style.height = 0 + 'px';
            let previousPlugHolder = previousPlug;
            setTimeout(() => {
                if (parseInt(previousPlugHolder.style.height, 10) === 0) {
                    previousPlugHolder.remove();
                    previousPlugHolder = false;
                }
            }, 210);

            //приваиваем переменной previousPlug ссылку на plug
            previousPlug = currentPlug;
        }, 15);
    ...
    let touchEndListener = function (event) {
        ...
        //set insertBeforeNote
        let insertBeforeNote = getInsertBeforeNoteByPlug(currentPlug, note.id);
        //undo top
        setTimeout(() => {
            note.style.top = getNoteTopCoordinate(currentPlug) + 'px';
            changeNoteWeightServer(note.id, insertBeforeNote ? insertBeforeNote.id : 0);

            setTimeout(() => {
                insertMovedNote(insertBeforeNote, note);
...
function getCurrentPlug(currentY, noteHeight, noteMarginTop, flowId, plugClass) {
    let elementToReturn = false;

    let elementsArray = [...document.getElementById(flowId).children];

    for (let i = 0; i < elementsArray.length; i++) {
        if (!elementsArray[i].classList.contains(plugClass)) {
            continue;
        }

        let plugBounding = elementsArray[i].getBoundingClientRect();

        if (
            plugBounding.top - noteHeight / 2 < currentY &&
            plugBounding.bottom + noteHeight / 2 + noteMarginTop > currentY ||
            (i === 0 && plugBounding.top > currentY) ||
            (i === elementsArray.length - 1 && plugBounding.bottom < currentY)
        ) {
            elementToReturn = elementsArray[i];
            break;
        }
    }
...
function getCurrentNote(currentX, currentY, idOfMovingNote) {
    let passedNote = false;

    let arrayWithNotes = [...document.elementsFromPoint(currentX, currentY)];

    arrayWithNotes.forEach(element => {
...
function extractNoteFromFlowJobAndCreateInsertGetPlug(note, notesCurrentArray, noteWidth, noteHeight, noteMarginTop, noteMarginBottom, topCoordinate) {
...
function getInsertBeforeElementByPassedNote(passedNote, currentY, notesCurrentArray) {
...
function insertPlugToFlow(flowId, passedNote, currentY, notesCurrentArray, plug, plugHeight) {

    let beforeElement = getInsertBeforeElementByPassedNote(passedNote, currentY, notesCurrentArray);

    if (beforeElement) {
        document.getElementById(flowId).insertBefore(plug, beforeElement);
    } else {
        document.getElementById(flowId).appendChild(plug);
    }

    setTimeout(() => {
        plug.style.height = (plugHeight) + 'px';
    }, 50);
}

function getInsertBeforeNoteByPlug(plug, movingNoteId) {
    let insertBeforeNote = false;

    let childNodes = [...document.getElementById(flowJob).childNodes];
    for (let i = 0; i < childNodes.length; i++) {
        if (
            childNodes[i] === plug
            && i !== childNodes.length - 1
            && childNodes[i + 1].classList.contains(cssNote)
            && childNodes[i + 1].id !== movingNoteId
        ) {
            insertBeforeNote = childNodes[i + 1];
        }
    }

    return insertBeforeNote;
...
function isCursorInBetweenNoteArea(passedNoteElement, currentY, noteHeight, noteMarginTop) {
    ...
    return (currentY < passedNoteElementTop + noteHeight / 2 && currentY > passedNoteElementTop - noteMarginTop) ||
        (currentY > passedNoteElementTop + noteHeight / 2 && currentY < passedNoteElementTop + noteHeight + noteMarginTop);
}
