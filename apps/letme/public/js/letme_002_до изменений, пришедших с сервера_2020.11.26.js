...
function moveTouchStartListener(note, id, startListener) {
    ...
    let touchStartX = isMobile() ? startListener.targetTouches[0].screenX : startListener.clientX;
    let touchStartY = isMobile() ? startListener.targetTouches[0].screenY : startListener.clientY;
    let currentY = 0;
    ...
    let moveListenerTimeout = setTimeout(() => {
    ...
    let moveListener = function (event) {
        ...
        currentY = isMobile() ? event.changedTouches[0].screenY : event.clientY;
        ...
        let passedNoteElementTemporary = getPassedNoteFromArray([...document.elementsFromPoint(100, currentY)], note.id);
        passedNoteElement = passedNoteElementTemporary ? passedNoteElementTemporary : passedNoteElement;

        movePlaceTimeout = setTimeout(() => {
            if (passedNoteElement && isCursorInBetweenNoteArea(passedNoteElement, currentY, noteHeight, noteMarginTop)) {
    ...
    document.body.addEventListener(typeOfMoveListener, moveListener);

    document.addEventListener(typeOfEndListener, (event) => {
        //undo preventDefault
        document.body.removeEventListener(typeOfMoveListener, moveListener, false);
